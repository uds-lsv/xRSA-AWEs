...
train_speech_df = speech_df[
    (speech_df.language.isin(label_set)) &
    (speech_df.num_ph>3) &
    (speech_df.frequency>1) &
    (speech_df.duration<1.10) &
    (speech_df.split=='TRA')
]#.sample(n=config_args['sample_size'])

valid_speech_df = speech_df[
    (speech_df.language.isin(label_set)) &
    (speech_df.num_ph>3) &
    (speech_df.frequency>1) &
    (speech_df.duration<1.10) &
...
speech_df = pd.concat([train_speech_df, valid_speech_df]) #32063 9228 0

print(len(speech_df))

#print(speech_df.head())
print('Number of spoken word sampels: ', len(speech_df))
print('Number of spoken word sampels in train: ', len(speech_df[(speech_df.split=='TRA')]))
print('Number of spoken word sampels in valid: ', len(speech_df[(speech_df.split=='DEV')]))
...
try:
    ...
    for epoch_index in range(num_epochs):
        ...
        for batch_index, batch_dict in enumerate(batch_generator):
            ...
            ank_embeddings, pos_embeddings = word_encoder(
            ...
            # get hard negatives from the mini-batch
            hard_negative_indicies = train_utils.get_hard_negatives(
                ank_embeddings,
                pos_embeddings,
                word_labels,
                negative_strategy=config_args['negative_strategy']
            )

            # compute loss
            loss = train_utils.triplet_loss_func(
                ank_embeddings,
                pos_embeddings,
                ank_embeddings[hard_negative_indicies],
                margin=config_args['training_hyperparams']['margin']
            )
        ...
        for batch_index, val_batch_dict in enumerate(batch_generator):
            ...
            # get vectors
            if batch_index == 0:
                acoustic_vectors = ank_embeddings.cpu().detach().numpy()
            ...
            # compute loss
            val_loss = train_utils.triplet_loss_func(
                ank_embeddings,
                pos_embeddings,
                ank_embeddings[hard_negative_indicies],
                margin=config_args['training_hyperparams']['margin']
